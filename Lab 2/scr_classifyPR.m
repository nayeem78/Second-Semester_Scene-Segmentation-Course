...
for i=1:length(d),
	...
	for j=1:2,
		...
		A = imread(name);
		vecTrain((i-1)*2+j,:) = [computeIntensityFeatureVector(A),computeFeatureVector(A)]; %training vector
		labTrain((i-1)*2+j) = i+48; %training labels
	end
	for j=3:length(d1),
		name = [dataDir namedir '/' d1(j).name];
		A = imread(name);
		vecTest((i-1)*4+j-2,:) = [computeIntensityFeatureVector(A),computeFeatureVector(A)]; %testing vector
